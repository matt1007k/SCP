      ...
      <v-flex xs12>
        <v-card class="rounded-xl pa-6">
          <v-container fluid>
            <v-layout row wrap>
              <v-flex xs12>
                <span class="display-1">Importar Personas y Actualizar Datos</span>
                <p>Actualizar los datos de las personas.</p>
              </v-flex>
            </v-layout>
          </v-container>
        </v-card>
...
export default {
  ...
  methods: {
    async sendFile() {
      ...
      try {
        this.uploading = true;
        const res = await axios.post("/importar/update-people", formData, {
          onUploadProgress: e =>
